...
struct f2fs_sb_info {
  ...
  unsigned int write_count[NR_CURSEG_TYPE];
#endif
...
struct f2fs_stat_info {
  ...
  unsigned int write_count[NR_CURSEG_TYPE * 2];
...
#define stat_inc_write_count(sbi, type)	((sbi)->write_count[type]++)
#define stat_dec_write_count(sbi, type)	((sbi)->write_count[type]--)
...
#define stat_inc_write_block_count(sbi, i)					\
	do {								\
		struct f2fs_stat_info *si = F2FS_STAT(sbi);		\
		si->write_count[i]++;				\
	} while (0)
#define stat_inc_meta_write_count(sbi)					\
	do {								\
		struct f2fs_stat_info *si = F2FS_STAT(sbi);		\
		si->meta_write_count++;					\
	} while (0)
