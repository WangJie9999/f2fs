...
static int stat_show(struct seq_file *s, void *v)
{
  ...
  int write_sum = 0;
    ...
    seq_printf(s, "Total LFS block write: %u\n", si->write_count[0] + si->write_count[2] + si->write_count[4] +
				si->write_count[6] + si->write_count[8] + si->write_count[10]);
		seq_printf(s, "Total SSR block write: %u\n", si->write_count[1] + si->write_count[3] + si->write_count[5] +
				si->write_count[7] + si->write_count[9] + si->write_count[11]);
   		write_sum = 0;
		for(j=0; j<6; j++)
			write_sum += si->write_count[j];
		write_sum -= si->data_blks;
		seq_printf(s, "\nData write count: %u\n", write_sum);
    write_sum = 0;
		for(j=6; j<12; j++)
			write_sum += si->write_count[j];
		write_sum -= si->node_blks;
		seq_printf(s, "Node write count: %u\n", write_sum); 
    seq_printf(s, "Meta write count: %u\n", si->meta_write_count);
